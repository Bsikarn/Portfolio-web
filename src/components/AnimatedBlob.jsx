...
export default function AnimatedBlob({ lowQuality }) {
  ...
  useFrame((state) => {
    if (lowQuality) return; // Prevent rotation updates in low quality mode
    if (meshRef.current) {
  ...
  return (
    <Float speed={lowQuality ? 0 : 2} rotationIntensity={lowQuality ? 0 : 0.2} floatIntensity={lowQuality ? 0 : 1}>
      {/* Place 3D model in the center, adjusted scale and position */}
      <primitive object={scene} ref={meshRef} scale={0.3} position={[0, -0.9, 0]} />

      {/* Background stars component within the 3D scene (rendered only in high quality) */}
      {!lowQuality && (
