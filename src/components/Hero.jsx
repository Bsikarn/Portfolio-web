    ...
    <section className="h-[calc(100vh-64px)] flex flex-col items-center justify-between px-[48px] py-[40px] max-w-[1440px] mx-auto relative z-1">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }} className="shrink-0 z-10 flex flex-col items-center gap-[32px]">
          ...
          <AnimatePresence mode="wait">
            {!isPdfOpen ? (
              ...
              >
                ...
                <motion.a href="#" target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-[32px] py-[14px] rounded-[50px] bg-brand-primary border-none text-white font-sans font-bold text-[15px] cursor-pointer shadow-hero-primary hover:shadow-hero-primary-hover no-underline inline-flex items-center">
                  Resume / CV
                </motion.a>
                <motion.a href="#" target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-[32px] py-[14px] rounded-[50px] bg-brand-primary border-none text-white font-sans font-bold text-[15px] cursor-pointer shadow-hero-primary hover:shadow-hero-primary-hover no-underline inline-flex items-center">
                  Portfolio
        ...
        <div className="flex flex-col items-center gap-[8px] opacity-70">
          <div className="font-sans text-[12px] font-semibold text-brand-muted uppercase tracking-[1px]">Scroll down to discover my journey</div>
          <motion.div
            animate={{ y: [0, 8, 0] }}
